...
如果我們同時知道起點 $X_0$ 與終點 $X_1$，
那麼最自然、最直覺的搬運方式，就是：

👉 **直接走直線**

也就是把「起點」和「終點」用一條直線連起來。
...
💡 數學補充： 這裡 $X_0$ 與 $X_1$ 是隨機變數，分別表示從起始分布與目標分布中抽取的樣本。因此，由它們所定義的線性插值 $X_t = (1 - t)X_0 + tX_1$
在固定 $t$ 下仍然是隨機變數，因為它依賴於隨機抽樣得到的 $(X_0, X_1)$，因此 $X_t$ 描述的是一個隨時間演化的隨機過程（stochastic process）。
...
在給定 $X_t$ 的情況下，$v_\theta(X_t,t)$ 和 $v(X_t, t)$ 都變成了常數向量，可以提出內積的期望值之外：
